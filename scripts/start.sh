...
start_jupyter() {
    mkdir -p "$APP_ROOT"
    echo "Starting JupyterLab on port 8048..."
    nohup jupyter lab --ServerApp.ip=0.0.0.0 --ServerApp.port=8048 --ServerApp.open_browser=false --ServerApp.token='' --ServerApp.password='' --ServerApp.root_dir="$APP_ROOT" &> /jupyter.log &
    echo "JupyterLab started on port 8048"
}
# ---------------------------------------------------------------------------- #
#                               Main Program                                     #
# ---------------------------------------------------------------------------- #
...
ARGS_FILE="$APP_ROOT/comfyui_args.txt"
if [ ! -f "$ARGS_FILE" ]; then
...
if [ -s "$ARGS_FILE" ]; then
    ...
    if [ ! -z "$CUSTOM_ARGS" ]; then
        ...
        nohup python main.py $FIXED_ARGS $CUSTOM_ARGS &> $APP_ROOT/comfyui.log &
    else
        echo "Starting ComfyUI with default arguments"
        nohup python main.py $FIXED_ARGS &> $APP_ROOT/comfyui.log &
    fi
else
