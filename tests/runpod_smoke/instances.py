...
# Lazy import sentinel — manifest._normalize_bool is used to interpret
# the per-group `check_all_gpu` field, but `manifest` already imports
# nothing from this module so there's no cycle risk; import directly.
from .manifest import _normalize_bool


# Extract CUDA version from image tag. Supports two conventions we ship:
#   * `runpod/base` / `runpod/pytorch` use cu(da)?MMmp — 3 digits, no dot
#     (e.g. `cuda1281` -> 12.8.1, `cu1300` -> 13.0.0)
#   * `runpod/comfyui` uses cudaMM.m — 2 digits + dot + 1 digit
#     (e.g. `cuda13.0`, `cuda12.8`)
# Returns "X.Y" suitable for --min-cuda-version, or None for images without
# an embedded CUDA version (CPU images, ROCm, NGC). Anchored with \b so we
# don't match e.g. 'cudnn'.
...
def detect_cuda_version(image: str) -> Optional[str]:
    ...
    or — worse — the pod boots on a CUDA-12.4 driver and the CUDA-13.0 torch
    inside the image silently fails to initialize, so the service never binds
    its port and the smoke check times out with no clear root cause.
    """
    m = CUDA_TAG_RE.search(image)
    if not m:
        return None
    major = m.group(1)
    # group(2)/(3) populated for cuda1281-style; group(4) for cuda13.0-style.
...
def _select_all_gpus(group_name: str, group_config: dict) -> list[str]:
    """Pick EVERY GPU from `config.GPU_CATALOG`, optionally filtered by
    `manufacturer` / `min_vram_gb`. Used when a group opts into
    `check_all_gpu: true` and doesn't pin an explicit `instances:` list.

    Sorted by displayName so the run order is stable across invocations —
    makes it easy to spot which GPUs flipped status between two runs.
    Returns [] (and logs a warn) when the catalog is empty, mirroring
    `_select_by_budget` — typically the case when `RUNPOD_API_KEY` isn't
    set.
    """
    if not config.GPU_CATALOG:
        log(
            f"warn: group '{group_name}' uses check_all_gpu but the "
            "GPU catalog is empty — set RUNPOD_API_KEY or use an explicit "
            "instances: list",
        )
        return []
...
def resolve_instances(group_name: str, group_config: dict) -> list[str]:
    ...
    Priority:
      ...
      3. `check_all_gpu: true` (+ optional `min_vram_gb`, `manufacturer`)
         ...
         `check_all_gpu` flag in runner.test_image to disable PASS short-
         circuiting). Lower priority than explicit / budget so users can
         still override on a per-group basis.
    ...
    Returns [] when nothing is set (caller will SKIP the group).
    """
