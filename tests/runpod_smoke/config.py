...
#     proxy (end-to-end reachability).
#
# Independent from `test_jupyter` — they can coexist. The Jupyter check is
# Jupyter-API-specific (server list + /api/status + token); these generic
...
#   * `instances.resolve_instances` returns the FULL GPU catalog (filtered
#     by manufacturer/min_vram_gb if set), instead of requiring an explicit
#     `instances:` list or a `max_price_per_hour:` budget.
#   * `runner.test_image` iterates EVERY resolved instance instead of
#     short-circuiting on the first PASS, so the run yields a per-(image,
#     instance) compatibility matrix.
#
# Designed for diagnostic runs ("find which GPUs this image works on") —
# NOT for CI. With ~30 NVIDIA types in the catalog and ~5 min per pod,
# a single image on check_all_gpu can take 2-3 hours and cost real money.
