...
def test_image(
    ...
    for inst in instances:
        ...
        if result == "UNAVAILABLE":
            # Remember which instances we tried but couldn't get. This is
            # what the summary shows in `[brackets]` when the final
            # outcome is SKIP — without it, `check_all_gpu` runs produce
            # a wall of identical "SKIP no capacity..." lines that don't
            # tell you which GPUs actually got refused.
    ...
    if stuck_instances:
        ...
        return (
            ...
            # Comma-joined so a single check_all_gpu row stays scannable;
            # default-mode usually has one instance here anyway. Caps at
            # the last instance label when the list is empty (defensive).
